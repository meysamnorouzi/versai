...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  <div className={cardBaseClasses}>
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-end gap-4 px-4 sm:px-6 md:px-8 pb-6 sm:pb-8 md:pb-10 opacity-0 pointer-events-none transition-opacity duration-300 delay-75 group-hover:opacity-100 group-hover:pointer-events-auto">
      <img
        className="pointer-events-none absolute left-1/2 top-[15%] h-36 w-48 -translate-x-1/2 rounded-2xl object-cover opacity-0 outline outline-2 outline-offset-[-1px] outline-stone-50 transition-all duration-500 ease-out group-hover:top-[-20%] group-hover:opacity-100 sm:h-40 sm:w-56 md:h-48 md:w-64"
        src={image}
        alt={title}
      />
      ...
      <Button
        ...
        size="md"
        className="w-full border-stone-50 text-stone-50 hover:bg-stone-50 hover:text-[#1e3950] hover:border-stone-50"
      >
        اطلاعات بیشتر
      </Button>
    </div>
